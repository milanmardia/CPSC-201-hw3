...
(define lookup
  (lambda (item table)
    (cond ((null? table) '(#f #f))
          ((equal? (first (first table)) item) (cons #t (rest (first table))))
...
(define unique-keys?
  (lambda (table)
    (let ((y (map first table)))
      (cond ((null? y) #t)
            (else (unique-keys-helper1 y))))))

(define unique-keys-helper1
  (lambda (lst)
    (cond ((null? lst) #t)
          ((unique-keys-helper2 (first lst) (rest lst)) #f)
          (else (unique-keys-helper1 (rest lst))))))

(define unique-keys-helper2
  (lambda (itm1 lst)
    (cond ((null? lst) #f)
          ((equal? itm1 (first lst)) #t)
          (else (unique-keys-helper2 itm1 (rest lst))))))
...
(define get-type
  (lambda (exp)
    (cond ((list? exp) (get-type-list exp))
          ((number? exp) 'constant)
          (else 'variable))))
...
(define eval-in-env
  (lambda (exp env)
    (cond ((not (list? exp)) (cond ((equal? (get-type exp) 'constant) exp)
                                   (else (first (rest (lookup exp env))))))
                                            
           ((equal? (length exp) 3) (evaluate (get-type exp) (eval-in-env (first (rest exp)) env) (eval-in-env (first (rest (rest exp))) env)))
           (else (evaluate1 (get-type exp) (eval-in-env (first (rest exp)) env))))))


(define evaluate
  (lambda (bool itm1 itm2)
    (cond ((equal? bool 'and) (band itm1 itm2))
          (else (bor itm1 itm2)))))
...
(eval-in-env ex1 environ1) 
(eval-in-env ex2 environ2) 
 (eval-in-env ex1 environ2)
 (eval-in-env ex3 environ2) 
 (eval-in-env '(* 1 0) '())
...
(define substitute
  (lambda (exp subst)
    (cond ((null? exp) '())
          ((not (list? exp)) (cond ((first(lookup exp subst)) (first (rest (lookup exp subst))))
                                   (else exp)))
          (else (cons (substitute (first exp) subst) (substitute (rest exp) subst))))))
...
(define is-variable?
  (lambda (itm)
    (and (symbol? itm) (not (or (or (equal? itm '+) (equal? itm '-)) (equal? itm '*))))))

(define all-variables-helper
  (lambda (exp)
    (cond ((null? exp) '())
          ((not(list? exp)) (if (is-variable? exp)
                                (list exp)
                                '()))
          (else (append (all-variables-helper (first exp)) (all-variables-helper (rest exp)))))))


(define all-variables-helper1
  (lambda (itm1 lst)
    (cond ((null? lst) #t)
          ((equal? itm1 (first lst)) #f)
          (else (all-variables-helper1 itm1 (rest lst))))))

(define all-variables-helper2
  (lambda (lst)
    (cond ((null? lst) '())
          ((all-variables-helper1 (first lst) (rest lst)) (cons (first lst) (all-variables-helper2 (rest lst))))
          (else (all-variables-helper2 (rest lst))))))

(define all-variables
  (lambda (exp)
    (let ((a (all-variables-helper exp)))
      (reverse (all-variables-helper2 (reverse a)) ))))
...
(map (lambda (x y) (list x y)) '(x y z) '(1 2 3) )
          
          
(define convert-to-binary
  (lambda (num)
    (cond ((equal? num 0) '())
          (else (cons (remainder num 2) (convert-to-binary (quotient num 2)))))))

(define convert-to-binary-real
  (lambda (num)
    (reverse (convert-to-binary num))))


(define create-list-of-binary-numbers
  (lambda (num)
    (cond ((< num 0) '())
          (else (cons (convert-to-binary-real num) (create-list-of-binary-numbers (- num 1)))))))
(define fix-list
  (lambda (lst1 lst2)
    (cond ((null? lst1) '())
          ((equal? (length (first lst1)) (length lst2)) (cons (first lst1) (fix-list (rest lst1) lst2)))
          (else (cons (fix-list1 (first lst1) lst2) (fix-list (rest lst1) lst2))))))

(define fix-list1
  (lambda (lst1 lst2)
    (cond ((equal? (length lst1) (length lst2)) lst1)
          (else (fix-list1 (cons 0 lst1) lst2)))))

(define create-list-of-lists-mid
  (lambda (num lst2)
    (fix-list (create-list-of-binary-numbers num) lst2)))
...
(define create-all-environments-final
  (lambda (lst)
    (let ((x (- (power 2 (length lst)) 1)))
      (reverse (create-all-environments-helper lst (create-list-of-lists-mid x lst))))))
      
(define create-all-environments-helper
  (lambda (lst1 lst2)
    (cond ((null? lst2) '())
          (else (cons (map (lambda (x y) (list x y)) lst1 (first lst2)) (create-all-environments-helper lst1 (rest lst2)))))))
...
(define satisfiable
  (lambda (exp)
    (cond ((number? exp)
                         (cond ((equal? exp 1) '())
                         (else #f)))
          ((equal? (length (all-variables exp) ) 1) (satisfiable-alternative exp))
          (else (satisfiable-helper exp (create-all-environments-final (all-variables exp)))))))

(define satisfiable-alternative
  (lambda (exp)
    (let ((x (all-variables exp)))
      (cond ((equal? (eval-in-env exp (cons (reverse (cons 0 x)) '())) 1) (cons (reverse (cons 0 x)) '()))
            ((equal? (eval-in-env exp (cons (reverse (cons 1 x)) '())) 1) (cons (reverse (cons 1 x)) '()))
            (else #f)))))


(define satisfiable-helper
  (lambda (exp env)
    (cond ((null? env) #f)
          ((equal? (eval-in-env exp (first env)) 1) (first env))
          (else (satisfiable-helper exp (rest env))))))
...
(define convert-bnand
  (lambda (exp)
    (cond ((null? exp) '())
          ((not (list? exp)) exp)
          ((equal? (get-type exp) 'not) (list '! (convert-bnand (first (rest exp))) (convert-bnand (first (rest exp))) ))
          ((equal? (get-type exp) 'or) (list '! (list '! (convert-bnand (first (rest exp))) (convert-bnand (first (rest exp)))) (list '! (convert-bnand (first (rest (rest exp)))) (convert-bnand (first (rest (rest exp)))))))
          (else (list '! (list '! (convert-bnand (first (rest exp))) (convert-bnand (first (rest (rest exp))))) (list '! (convert-bnand (first (rest exp))) (convert-bnand (first (rest (rest exp))))))))))
